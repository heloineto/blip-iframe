...
- Header/footer can now be collapsed the same way as navbar/aside. For example, header can be collapsed based on scroll position or direction.
...
- `actions` prop now uses different data format
...
New [Combobox](/core/combobox) component allows building custom select, autocomplete, tags input, multiselect and other
...
- [Checkbox](/core/checkbox), [Radio](/core/radio), [Chip](/core/chip) and [Switch](/core/switch) components now support `rootRef` prop which allows to use them with [Tooltip](/core/tooltip) and other similar components
