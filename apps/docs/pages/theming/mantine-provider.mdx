...
`MantineThemeProvider` allows overriding [theme](/theming/theme-object) values for a subtree of components.
Note that values that are used to generate [CSS variables](/styles/css-variables) cannot be overridden with `MantineThemeProvider`.
For example, you cannot use `MantineThemeProvider` to override `colors` or `fontSizes`.

<Demo data={ThemingDemos.themeProvider} />
