...
state or feature. For example, when `disabled` prop on [Button](/core/button) is set
`data-disabled` attribute is added to the `<button />` element:
...
When `disabled` prop is not set, then `data-disabled` attribute is not added to the
button:
...
In some cases, `data-*` attributes have associated values, for example, [Button](/core/button)
component `section` element has an associated `data-position` attribute which can be
...
<ModifiersTable data={ButtonStylesApi} withTableBorder={false} my="xl" />
...
[Box](/core/box) component supports `mod` prop, which allows adding data attributes to
